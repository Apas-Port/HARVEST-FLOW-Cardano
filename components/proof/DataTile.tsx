...
type TileValueProps = {
  value?: string;
  size?: "xxs" | "xs" | "xs24" | "small" | "large" | "xxs-account-project-term";
};
...
const TileValue: React.FC<TileValueProps> = ({ value, size = "small" }) => {
  const getPaddingFromSize = () => {
    ...
    if (size === "xxs-account-project-term") {
      return "p-2";
    }
  ...
  const getTextSizeFromSize = () => {
    if (size === "xxs") {
      return "text-bodyLarge xl:text-heading5LH22";
    }
    ...
    if (size === "xs") {
      return "text-bodyLarge xl:text-heading5";
    }

    if (size === "xs24") {
      return "text-bodyLarge24 xl:text-heading4Smaller";
    }

    if (size === "large") {
      return "text-heading3 xl:text-heading2_43";
    }

    return "text-body xl:text-heading3";
  };

  if (!value) return null;

  return (
    <div className={clsx("flex items-center justify-center flex-1", getPaddingFromSize())}>
      <p className={clsx("text-center uppercase", getTextSizeFromSize())}>{value}</p>
    </div>
  );
};
  ...
  return (
    <div className="px-3 xl:px-6 py-4 xl:py-[10px] border-b border-black border-dashed flex items-center justify-center">
      <p className="text-center uppercase text-captionMedium xl:text-caption relative">
        ...
        {tooltipText && (
          <>
            <div className="absolute w-[12px] h-[12px] flex items-center justify-center top-0 -right-4 bottom-0 my-auto hover:cursor-pointer" title={tooltipText}>
              <TooltipIcon />
            </div>
          </>
        )}
...
const DataTile: React.FC<DataTileProps> = ({ title, tooltipText, value, size, className, children }) => {
  return (
    <div className={clsx("border-t border-l border-black flex flex-col", className)}>
      <TileHeader title={title} tooltipText={tooltipText} />
      <div className="flex items-center justify-center flex-1">{children ?? <TileValue value={value} size={size} />}</div>
    </div>
  );
};
